...
func TestDrainSchedules_Schedule(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "fourth schedule (following 5m custom drain buffer)",
			node: &v1.Node{ObjectMeta: meta.ObjectMeta{Name: nodeName + "4"}},
			window: timeWindow{
				from: whenFirstSched.Add(2*period + 5*time.Minute - 2*time.Second),
				to:   whenFirstSched.Add(2*period + 5*time.Minute + 2*time.Second),
			},
		},
	}
